...
public class DashboardServiceImpl implements StudentDashboardService {
    ...
    public DashboardResponse getDashboard(Long studentId, LocalDate date) {
        ...
        // Attendance: current week (Mon-Sun)
        ...
        List<DashboardResponse.NoticeItem> noticeItems = recentNotices.stream()
                .map(n -> new DashboardResponse.NoticeItem(
                        ...
                        !isRead(studentId, n.getId(), noticesUnread) // simple UI hint; you can refine
                ))
                .toList();
    ...
    // keep dashboard service pure; if you want perfect unread per item, move to NoticeService
    private boolean isRead(Long studentId, Long noticeId, int unreadCount) {
        return false;
    }
}
